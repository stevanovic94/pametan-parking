...
import { ParkingLotsService } from "../parking-lots/parking-lots.service.js";
import { PrismaService } from "../prisma/prisma.service.js";
import { ReservationStatus } from "../generated/prisma/client.js";

import { CreateParkingSpaceDto } from "./dto/create-parking-space.dto.js";
import { ParkingSpacesQueryDto } from "./dto/parking-spaces-query.dto.js";
import { UpdateOccupancyDto } from "./dto/update-occupancy.dto.js";
import { UpdateParkingSpaceDto } from "./dto/update-parking-space.dto.js";
...
export class ParkingSpacesService {
	...
	async create(dto: CreateParkingSpaceDto) {
		 ...
		 * i da je aktivan.
		 */
		await this.parkingLotsService.findOne(dto.parkingLotId);

		const code = this.normalizeCode(dto.code);

		await this.ensureCodeAvailable(dto.parkingLotId, code);
	...
	async findAll(query: ParkingSpacesQueryDto) {
		const now = new Date();
		...
		return parkingSpaces.map(({ reservations, ...parkingSpace }) => ({
			...parkingSpace,

			isReservedNow: reservations.length > 0,
		}));
	}

	findAllForAdmin(query: ParkingSpacesQueryDto) {
	...
	async findOne(id: string) {
		...
		if (!parkingSpace) {
			throw new NotFoundException("Parking mesto nije pronađeno.");
		}
	...
	async update(id: string, dto: UpdateParkingSpaceDto) {
		...
		if (dto.isActive === true) {
			await this.parkingLotsService.findOne(existing.parkingLotId);
		}
	...
	async deactivate(id: string) {
		...
		const parkingSpace = await this.prisma.parkingSpace.update({
			where: {
				id,
			},
		...
		return {
			message: "Parking mesto je deaktivirano.",

			parkingSpace,
		};
	}

	async updateOccupancy(id: string, dto: UpdateOccupancyDto) {
		/*
