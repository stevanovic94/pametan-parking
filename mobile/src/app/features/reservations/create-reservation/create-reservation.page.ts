...
export class CreateReservationPage {
	...
	readonly parkingSpaceId =
		this.route.snapshot.paramMap.get("parkingSpaceId") ?? "";
		...
		this.parkingService
			...
			.subscribe({
				...
				error: () => {
					...
					this.serverError.set("Parking mesto nije pronađeno.");
				},
			});
	}
	...
	submit(): void {
		...
		if (Number.isNaN(startAt.getTime()) || Number.isNaN(endAt.getTime())) {
			this.serverError.set("Unesite ispravan datum i vreme.");
			return;
		}

		if (startAt >= endAt) {
			this.serverError.set("Vreme početka mora biti pre vremena završetka.");
			return;
		}

		if (startAt <= new Date()) {
			this.serverError.set("Rezervacija mora početi u budućnosti.");
			return;
		}
		...
		this.reservationService
			...
			.subscribe({
				...
				error: (error: HttpErrorResponse) => {
					...
					if (error.status === 400) {
						this.serverError.set("Izabrani termin nije ispravan.");
						return;
					}

					if (error.status === 404) {
						this.serverError.set("Parking mesto više nije dostupno.");
						return;
					}

					this.serverError.set("Kreiranje rezervacije nije uspelo.");
				},
			});
	}

	occupancyLabel(): string {
		switch (this.parkingSpace()?.occupancyStatus) {
			case "FREE":
				return "Slobodno";
